<?php
namespace FireGento\Pdf\Helper;

use \FireGento\Pdf\Model\System\Config\Source;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @return mixed
     */
    public function putOrderId(Order $order, $mode = 'invoice')
    {
        switch ($mode) {
            case 'invoice':
                $putOrderIdOnInvoice = Mage::getStoreConfigFlag(
                    AbstractPdf::XML_PATH_SALES_PDF_INVOICE_PUT_ORDER_ID,
                    $order->getStoreId()
                );
            ...
            case 'shipment':
                $putOrderIdOnShipment = Mage::getStoreConfigFlag(
                    AbstractPdf::XML_PATH_SALES_PDF_SHIPMENT_PUT_ORDER_ID,
                    $order->getStoreId()
                );
            ...
            case 'creditmemo':
                $putOrderIdOnCreditmemo = Mage::getStoreConfigFlag(
                    AbstractPdf::XML_PATH_SALES_PDF_CREDITMEMO_PUT_ORDER_ID,
                    $order->getStoreId()
                );
     ...
     *
     * @param  \Magento\Framework\Model\AbstractModel $model the model instance
     *
        ...
        if (!$model instanceof Order) {
            switch ($model) {
                case $model instanceof Invoice:
                    ...
                    break;
                case $model instanceof Shipment:
                    $specificVars = array(
                        '{{shipment_id}}' => $model->getIncrementId()
                    );
                    break;
                case $model instanceof Creditmemo:
                    $specificVars = array(
